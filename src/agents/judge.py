...
from utils.utils import tokenCounter
from config.prompt import CRITERIA_BASED_JUDGING_PROMPT, NLI_PROMPT
...
class Judge:
    ...
    def criteria_based_judging(self, survey, topic, criterion):
        ...
        content_paras = {
            ...
            "Score 1 Description": criterion_paras["score1"],
            "Score 2 Description": criterion_paras["score2"],
            "Score 3 Description": criterion_paras["score3"],
            "Score 4 Description": criterion_paras["score4"],
            "Score 5 Description": criterion_paras["score5"],
        }
    ...
    def citation_quality(self, survey_with_reference, references):
        ...
        index_to_titles = {
            int(index): ids_to_title[idx] for index, idx in references.items()
        }
        ...
        for i in range(len(claims)):
            ...
            thread = threading.Thread(
                target=self.__nli, args=(sources, claims[i], scores, i)
            )
            thread_l.append(thread)
            thread.start()
        ...
        for j, claim, source_ids in zip(range(len(claims)), claims, sources_ids):
            ...
            if scores[j] == 1:
                for index in source_ids:
                    ...
                    com_sources = [
                        index_to_paper[_] for _ in source_ids if not _ == index
                    ]
                    thread = threading.Thread(
                        target=self.__relevant,
                        args=(sources, com_sources, claim, precisions, j),
                    )
                    thread_l.append(thread)
                    thread.start()
        for thread in tqdm(thread_l):
